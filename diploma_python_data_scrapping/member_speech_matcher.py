...
def get_gov(current_record_datetime):
    ...
    df_govs = pd.read_csv(
        "C:/Users/johnp/Documents/ECE_NTUA/diploma/dipoma_code/diploma_python_data_scrapping/governments_1989onwards.csv", encoding='utf-8')
    ...
    for i in range(len(speakers)):
        ...
        if general_member_regex.search(speaker):
            ...
            if 'εφηβοι' in speaker:
                ...
                roles = np.nan
                print("\n\n----------------------\n\n")
                print([speaker, current_record_datetime.strftime('%d/%m/%Y'),
...
df['parliamentary_period'].replace(
    {"-presided-parliamentary-republic_": '_'}, inplace=True, regex=True)

# Correct order of sitting date column
df['sitting_date'].replace(
    {"-presided-parliamentary-republic_": '_'}, inplace=True, regex=True)
